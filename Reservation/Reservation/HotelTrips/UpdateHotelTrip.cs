...
{
    // Updates a Hotel-Trip booking status by DocumentId. Deploy as:
    // Reservation.HotelTrips.UpdateHotelTrip, Reservation, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
    public class UpdateHotelTrip : ActivityTemplate
    {
        ...
        private static void ProcessWorkflowItem(WorkflowItem workflowItem)
        {
            ...
            try
            {
                ...
                var request = new UpdateHotelTripBookingRequest
                {
                    DocumentId = currentDocumentId,
                    BookingStatus = bookingStatus
                };

                var updateUrl = BuildUrl(reservationUrl, "/api/HotelTrip/update-booking");
            ...
            catch (Exception ex)
            {
                ...
                Intalio.Core.ExceptionLogger.WriteEntry(
                    $"Exception in UpdateHotelTrip Code Activity: {ex.Message}\nStack Trace: {ex.StackTrace}");
                throw;
            }
        }
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                throw new InvalidOperationException(
                    !string.IsNullOrWhiteSpace(apiMessage)
                        ? apiMessage
                        : $"Update hotel-trip booking failed with status {(int)response.StatusCode}. Response: {responseBody}");
            }
        ...
        private sealed class UpdateHotelTripBookingRequest
        {
