...
namespace Reservation.HotelTrips
{
    // Submits a Hotel-Trip booking to the ECM API. Deploy as:
    // Reservation.HotelTrips.HotelTrip, Reservation, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
    public class HotelTrip : ActivityTemplate
    {
        ...
        private static void ProcessWorkflowItem(WorkflowItem workflowItem)
        {
            ...
            try
            {
                ...
                var submitUrl = BuildUrl(reservationUrl, "/api/HotelTrip/bookings");
            ...
            {
                if (ex is HotelTripBookingFailedException submitFailure)
                {
                ...
                Intalio.Core.ExceptionLogger.WriteEntry(
                    $"Exception in HotelTrip Code Activity: {ex.Message}\nStack Trace: {ex.StackTrace}");
                throw;
            }
        }
        ...
        private static HotelTripBookingSubmitRequest BuildSubmitRequest(JsonElement root, long workflowId, long documentId)
        {
            return new HotelTripBookingSubmitRequest
            {
                HotelTripId = GetIntValue(root, "trip") ?? 0,
        ...
        private static string SendPostRequest(string url, string requestBody)
        {
            ...
            {
                throw new HotelTripBookingFailedException(
        ...
        private static string BuildFailureMessage(int statusCode, string responseBody)
        {
            ...
            return !string.IsNullOrWhiteSpace(apiMessage)
                ? apiMessage
                : $"Hotel-trip booking failed with status {statusCode}. Response: {responseBody}";
        }
        ...
        private sealed class HotelTripBookingSubmitRequest
        {
        ...
        private sealed class HotelTripBookingFailedException : InvalidOperationException
        {
            public HotelTripBookingFailedException(string message, string responseBody)
                : base(message)
            {
